...
const DATA=[
  {rank:1,team:"Innovators Hub",score:98.5,track:"EdTech",pts:450,event:"Spring Hackathon 2026",members:4},
  {rank:2,team:"Code Crafters",score:95.0,track:"Open",pts:410,event:"Spring Hackathon 2026",members:3},
  {rank:3,team:"Data Miners",score:92.2,track:"AI/ML",pts:395,event:"Winter Ideathon 2025",members:4},
  {rank:4,team:"Campus Fix",score:89.0,track:"Sustainability",pts:350,event:"Spring Hackathon 2026",members:2},
  {rank:5,team:"Byte Me",score:85.5,track:"Open",pts:320,event:"Winter Ideathon 2025",members:3},
];
const EVENTS=["All","Spring Hackathon 2026","Winter Ideathon 2025"];

const rankCls={1:"bg-yellow-100 text-yellow-700 ring-2 ring-yellow-300",2:"bg-slate-200 text-slate-700",3:"bg-amber-100 text-amber-800"};

export default function LeaderboardPage(){
  const [ev,setEv]=useState("All");
  const filtered=ev==="All"?DATA:DATA.filter(d=>d.event===ev);
    ...
    <div className="max-w-7xl mx-auto px-6 md:px-12 py-12">
      <div className="relative rounded-3xl overflow-hidden mb-12 h-60 shadow-xl">
        ...
        <div className="absolute inset-0 flex flex-col justify-center px-10 text-white">
          ...
          <p className="text-slate-300 font-medium mt-2">Top performing teams across all RK University hackathons.</p>
        </div>
      </div>
      ...
      <div className="bg-gradient-to-br from-indigo-900 via-slate-900 to-slate-900 text-white rounded-3xl p-8 shadow-2xl relative overflow-hidden flex flex-col items-center mb-10">
        ...
        <div className="flex items-end gap-6 h-52 relative z-10">
          {[{...top3[1],h:"h-28",c:"from-slate-300 to-slate-400",tc:"text-slate-200"},{...top3[0],h:"h-40",c:"from-yellow-300 to-orange-400",tc:"text-yellow-300"},{...top3[2],h:"h-24",c:"from-amber-600 to-amber-800",tc:"text-slate-300"}].map((p,i)=>(
            <motion.div key={p.rank} initial={{y:60,opacity:0}} animate={{y:0,opacity:1}} transition={{delay:i*0.15}} className="flex flex-col items-center">
              ...
              <div className={`w-32 ${p.h} bg-white/10 backdrop-blur-md rounded-t-2xl border-t border-x border-white/20 flex flex-col items-center justify-end pb-4 px-2`}>
                ...
                <p className="text-xs text-slate-400 mt-1">{p.score}</p>
              </div>
            </motion.div>
          ))}
        </div>
      ...
      <div className="flex items-center gap-3 mb-6 flex-wrap">
        ...
        {EVENTS.map(e=><button key={e} onClick={()=>setEv(e)} className={`px-4 py-2 rounded-xl text-sm font-bold transition-all ${ev===e?"bg-sky-500 text-white shadow-md":"bg-white border border-slate-200 text-slate-600 hover:bg-slate-50"}`}>{e}</button>)}
      </div>
            ...
            <tbody>
              {filtered.map((t,i)=>(
                <motion.tr key={i} initial={{opacity:0,x:-10}} animate={{opacity:1,x:0}} transition={{delay:i*0.05}}
                  className={`border-b border-slate-100 hover:bg-slate-50/70 transition-colors ${i===0&&ev==="All"?"bg-yellow-50/40":""}`}
                >
                  <td className="table-cell"><span className={`w-9 h-9 rounded-full flex items-center justify-center font-black text-sm ${rankCls[t.rank]||"bg-slate-50 text-slate-400 border border-slate-200"}`}>{t.rank}</span></td>
                  ...
                  <td className="table-cell font-black text-slate-900 text-right text-lg">{t.score.toFixed(1)}</td>
                  <td className="table-cell text-right"><span className="inline-flex items-center gap-1 font-black text-sky-500 bg-sky-50 border border-sky-200 px-3 py-1.5 rounded-lg"><Star size={12} className="fill-sky-500"/>{t.pts}</span></td>
                </motion.tr>
              ))}
            </tbody>
          </table>
