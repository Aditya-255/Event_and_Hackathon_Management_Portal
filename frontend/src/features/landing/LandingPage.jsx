...
export default function LandingPage() {
  ...
  useEffect(() => {
    eventsAPI.getAll()
      .then(({ data }) => setEvents((data || []).filter(e => e.status !== "Completed" && e.status !== "Event Over").slice(0, 4)))
      .catch(() => {});
    leaderboardAPI.global()
      .then(({ data }) => setHallOfFame((data || []).slice(0, 3)))
      .catch(() => {});
  }, []);
  ...
  return (
    <div className="pb-20">
      {/* Hero */}
      <section className="relative min-h-[95vh] flex items-center overflow-hidden" style={{ paddingTop: 64 }}>
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .6 }} className="relative z-10 max-w-3xl px-8 md:px-16 py-20">
          <div className="inline-flex items-center gap-2 bg-white/10 backdrop-blur-md border border-white/20 text-white px-4 py-2 rounded-full text-sm font-bold mb-6 shadow-lg">
            <Zap size={14} className="text-yellow-300" />{events.length || 3} events live right now
          </div>
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        ...
        <section className="mb-20">
          ...
          {events.length === 0 ? (
            <div className="text-center py-16 text-slate-400 font-semibold">Loading events...</div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {events.map((ev, i) => {
                const teamsCount = parseInt(ev.teams_count || 0);
                const startDate  = ev.start_date ? new Date(ev.start_date).toLocaleDateString("en-IN", { day: "numeric", month: "short" }) : "TBD";
                return (
                  ...
                  >
                    ...
                    <div className="relative z-10 flex justify-between items-start p-5">
                      <span className={`px-4 py-1.5 rounded-full text-xs font-black backdrop-blur-md border ${statusCls[ev.status] || "bg-sky-500/20 text-sky-300 border-sky-500/30"}`}>{ev.status}</span>
                    ...
                    <div className="mt-auto relative z-10">
                      <div className="m-3 p-5 rounded-2xl bg-white/10 backdrop-blur-xl border border-white/20 text-white">
                        ...
                        <div className="flex justify-between items-center text-sm font-semibold text-slate-200 border-t border-white/10 pt-4 mt-3">
                          <span className="flex items-center gap-2"><Calendar size={15} />{startDate}</span>
                          <span className="flex items-center gap-2"><Users size={15} />{teamsCount} teams</span>
                          <span className="flex items-center gap-2"><Trophy size={15} />{ev.prize || "TBD"}</span>
                        </div>
                      </div>
                    </div>
        ...
        <section className="mb-20">
          <div className="flex items-center gap-4 mb-8">
            ...
            <div><h2 className="text-3xl font-black text-slate-900">Hall of Fame</h2><p className="text-slate-500 font-medium">Top ranked teams</p></div>
          </div>
          <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
            <table className="w-full text-left">
              <thead><tr className="bg-slate-50 border-b border-slate-200">{["Rank", "Team", "Event", "Score", ""].map(h => <th key={h} className="table-head">{h}</th>)}</tr></thead>
              <tbody>
                {hallOfFame.length === 0 ? (
                  <tr><td colSpan={5} className="table-cell text-center text-slate-400 py-8">Loading rankings...</td></tr>
                ) : hallOfFame.map((p, i) => (
                  <tr key={p.id || i} className={`border-t border-slate-100 hover:bg-slate-50/50 transition-colors ${i === 0 ? "bg-amber-50/30" : ""}`}>
                    <td className="table-cell">
                      <span className={`w-8 h-8 rounded-full flex items-center justify-center font-black text-sm ${i === 0 ? "bg-yellow-100 text-yellow-700 ring-2 ring-yellow-300" : i === 1 ? "bg-slate-200 text-slate-700" : "bg-amber-100 text-amber-800"}`}>{p.rank}</span>
                    </td>
                    <td className="table-cell">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 bg-amber-100 rounded-full flex items-center justify-center text-base">🏆</div>
                        <span className="font-semibold text-slate-700">{p.team}</span>
                      </div>
                    </td>
                    <td className="table-cell text-sm text-slate-500 font-medium">{p.event}</td>
                    <td className="table-cell">
                      <div className="flex items-center gap-3">
                        <div className="w-24 h-2 bg-slate-100 rounded-full overflow-hidden">
                          <div className="h-full bg-gradient-to-r from-sky-500 to-violet-500 rounded-full" style={{ width: `${p.score}%` }} />
                        </div>
                        <span className="font-black text-slate-900">{parseFloat(p.score).toFixed(1)}</span>
                      </div>
                    </td>
