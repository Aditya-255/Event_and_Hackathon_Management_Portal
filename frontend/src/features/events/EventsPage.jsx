...
import { Search, Filter, Calendar, Users, ChevronRight, X, Trophy } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
import { Link } from "react-router-dom";
import axios from "axios";

const CATS = ["All", "Tech", "Design", "AI/ML"];
const STATS = ["All", "Registration Open", "Registration Full", "Event Over"];
    ...
    <div className="max-w-7xl mx-auto px-6 md:px-12 py-12">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-10 gap-6">
        ...
        <div className="flex gap-3 w-full md:w-auto">
          ...
          <button onClick={()=>setShowF(!showF)} className={`px-4 py-3 border rounded-xl flex items-center gap-2 font-bold text-sm transition-all ${showF||fc>0?"bg-sky-500 text-white border-sky-500 shadow-md":"bg-white border-slate-200 text-slate-700 hover:bg-slate-50"}`}>
            ...
            >
              ...
              <div className="mt-auto relative z-10">
                <div className="m-3 p-5 rounded-2xl bg-white/10 backdrop-blur-xl border border-white/20 text-white group-hover:bg-white/15 transition-all">
                  ...
                  {ev.status!=="Event Over"&&(
                    <div className="mb-4">
                      <div className="flex justify-between text-xs font-semibold text-slate-300 mb-1.5">
                        <span>{ev.teams}/{ev.maxTeams} teams</span>
                        <span>{Math.round((ev.teams/ev.maxTeams)*100)}% full</span>
                      </div>
                      <div className="w-full h-1.5 bg-white/20 rounded-full overflow-hidden">
                        <div className={`h-full rounded-full ${ev.status==="Registration Full"?"bg-red-400":"bg-emerald-400"}`} style={{width:`${(ev.teams/ev.maxTeams)*100}%`}}/>
                      </div>
                    </div>
                  )}
                  <div className="flex justify-between text-sm font-semibold text-slate-200 border-t border-white/10 pt-4">
                    <span className="flex items-center gap-1.5"><Calendar size={14}/>{ev.date}</span>
